...
thus leaving the resource unprotected. Aside channel exposes
...
USENIX Association 28th USENIX Security Symposium    603
...
604    28th USENIX Security Symposium USENIX Association
...
USENIX Association 28th USENIX Security Symposium    605
...
allows app1 access to resources but deniesapp2 access; this is
...
606    28th USENIX Security Symposium USENIX Association


## Strona 6

requires building an instrumentation framework for possible
...
losophy. We also logged whenever anioctl was issued to the
...
to the UID as reported by theproc ﬁlesystem. Furthermore, it
...
608    28th USENIX Security Symposium USENIX Association
...
Decoding Obfuscations In our previous work [66], we
...
USENIX Association 28th USENIX Security Symposium    609
...
Data Type Permission Purpose/Use Subsection N o of Apps N o of SDKs Channel Type
Dynamic Static Dynamic Static Covert Side
IMEI READ_PHONE _STATE Persistent ID 4.1 13 159 2 2 2 0
Device MAC ACCESS_NETWORK _STATE Persistent ID 4.2 42 12,408 1 1 0 1
Email GET_ACCOUNTS Persistent ID Not Found
Phone Number READ_PHONE _STATE Persistent ID Not Found
SIM ID READ_PHONE _STATE Persistent ID Not Found
...
READ _PHONE _STATE permission. We identiﬁed two third-
...
ers [67]. We identiﬁed network ﬂows tosalmonads.com com-
...
is protected with the ACCESS _NETWORK _STATE permission.
...
phone’s Android ID.
Baidu uses AES in CBC mode with a static
key and the same static value for the initializa-
...
the READ _PHONE _STATE permission. This means that they
...
the ACCESS _NETWORK _STATE permission. Despite this, we
...
address to Unity’s servers and referring to it as auuid in the
transmission (e.g., as an HTTP GET parameter key name).
In this case, the access was happening inside of Unity’s C++
native library. We reverse engineeredlibunity.so to deter-
...
SIOCGIFCONF 2 ioctl to get the network interfaces, and then
uses the SIOCGIFHWADDR 3 ioctl to get the corresponding
MAC address.
We observed that 42 apps were obtaining and sending to
Unity servers the MAC address of the network card with-
out holding the ACCESS _NETWORK _STATE permission. To
...
CESS _NETWORK _STATE permission.
...
by the ACCESS _WIFI _STATE permission. In Section 2, we
...
just those without the ACCESS _WIFI _STATE permission. This
...
CESS _WIFI _STATE and ACCESS _COARSE _LOCATION per-
...
SDK; those apps that do not have ACCESS _WIFI _STATE, which means that the side channel circumvents the permissions system;
and those apps which do have a location permission, which means that the side channel circumvents location revocation.
Contact Incorporation Total Prevalance Wi-Fi Permission No Location Permission
SDK Name Domain Country (Apps) (Installs) (Apps) (Installs) (Apps) (Installs)
...
USENIX Association 28th USENIX Security Symposium    613
...
READ _EXTERNAL _STORAGE ) can learn the user’s precise
...
consent” framework: companies can givenotice to consumers
...
614    28th USENIX Security Symposium USENIX Association
...
616    28th USENIX Security Symposium USENIX Association
...
USENIX Association 28th USENIX Security Symposium    617
...
618    28th USENIX Security Symposium USENIX Association
...
USENIX Association 28th USENIX Security Symposium    619
...
620    28th USENIX Security Symposium USENIX Association
